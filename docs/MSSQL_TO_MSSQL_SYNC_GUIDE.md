...
این راهنما نحوهٔ افزودن یک DAG جدید برای **همگام‌سازی (replication-style)** از یک **SQL Server** به **SQL Server** دیگر را توضیح می‌دهد. داده با یک query از منبع خوانده می‌شود و با **staging MERGE upsert** در جدول مقصد نوشته می‌شود.

...
این راهنما نحوهٔ افزودن یک DAG جدید برای **همگام‌سازی (replication-style)** از **SQL Server** به **MySQL** را توضیح می‌دهد. داده با یک query از MSSQL خوانده می‌شود و با **staging upsert** در جدول مقصد MySQL نوشته می‌شود.

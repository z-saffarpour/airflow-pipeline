"""
Airflow DAG: ax.RetailDiscountLineOffer Query to Store
===============================================
This DAG executes a business query on ax.RetailDiscountLineOffer and sends results to Store.
...
dag_config = DAGConfig(
    ...
    description = 'masterdata query sync from ax.RetailDiscountLineOffer to Store',
    ...
    retries=int(Variable.get("retries_retail_discount_line_offer", default_var=2)),
    retry_delay=timedelta(minutes=int(Variable.get("retry_delay_minutes_retail_discount_line_offer", default_var=5))),
    execution_timeout=timedelta(hours=int(Variable.get("execution_timeout_hours_retail_discount_line_offer", default_var=8))),
    # Tags
...
sync_config =  MasterDataSyncConfig(
    ...
    chunk_column='RECID',
    task_chunk_size=int(Variable.get("task_chunk_size_retail_discount_line_offer", default_var=100000)),
    max_parallel_chunks=int(Variable.get("max_parallel_chunks_retail_discount_line_offer", default_var=8)),
    max_global_parallel_chunks=int(Variable.get("max_global_parallel_chunks_replication_md_store", default_var=32)),

    target_schema = 'ax',
    target_table = 'RetailDiscountLineOffer',

...
publisher (HQ) for PRICEDISCTABLE articles, detects row-count inconsistencies,
...
DAG_RETRIES = int(
    Variable.get("retries_masterdata_replication_reconcile_and_sync", default_var=0)
)
DAG_RETRY_DELAY = timedelta(
    minutes=int(
        Variable.get("retry_delay_minutes_masterdata_replication_reconcile_and_sync", default_var=5)
    )
)
DAG_EXECUTION_TIMEOUT = timedelta(
    hours=int(
        Variable.get("execution_timeout_hours_masterdata_replication_reconcile_and_sync", default_var=8)
    )
)
...
def _get_subscriber_servers() -> List[Dict[str, Any]]:
    ...
    if not rows:
        raise AirflowFailException(
            "No active connection info found."
        )
    servers = [
        {
            "ip": row["ServerIP"],
...
def _generate_subscriber_reconcile_script() -> Tuple[str, int]:
    ...
    SET @mySubscriberScript = @mySubscriberScript + 'WHERE PublicationRowCount - SubscriberRowCount > 1'
...
def trigger_sync_orchestrators_task(
    ...
    **context,
    ...
    for chunk in reconcile_results:
...
with DAG(
    dag_id="masterdata_replication_reconcile_and_sync",

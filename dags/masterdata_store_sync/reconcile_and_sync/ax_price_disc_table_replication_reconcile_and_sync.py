...
DAG_RETRIES = int(
    Variable.get("retries_ax_price_disc_table_replication_reconcile_and_sync", default_var=0)
)
DAG_RETRY_DELAY = timedelta(
    minutes=int(
        Variable.get("retry_delay_minutes_ax_price_disc_table_replication_reconcile_and_sync", default_var=5)
    )
)
DAG_EXECUTION_TIMEOUT = timedelta(
    hours=int(
        Variable.get("execution_timeout_hours_ax_price_disc_table_replication_reconcile_and_sync", default_var=8)
    )
)
...
with DAG(
    dag_id="ax_price_disc_table_replication_reconcile_and_sync",

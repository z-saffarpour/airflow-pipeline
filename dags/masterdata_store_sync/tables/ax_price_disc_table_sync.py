...
dag_config = DAGConfig(
    ...
    description = 'masterdata query sync from ax.PriceDiscTable to Store',
    ...
    retries=int(Variable.get("retries_price_disc_table", default_var=2)),
    retry_delay=timedelta(minutes=int(Variable.get("retry_delay_minutes_price_disc_table", default_var=5))),
    execution_timeout=timedelta(hours=int(Variable.get("execution_timeout_hours_price_disc_table", default_var=8))),
    # Tags
...
sync_config =  MasterDataSyncConfig(
    ...
    chunk_column='RECID',
    task_chunk_size=int(Variable.get("task_chunk_size_price_disc_table", default_var=100000)),
    max_parallel_chunks=int(Variable.get("max_parallel_chunks_price_disc_table", default_var=8)),
    max_global_parallel_chunks=int(Variable.get("max_global_parallel_chunks_replication_md_store", default_var=32)),

    target_schema = 'ax',
    target_table = 'PriceDiscTable',
    staging_schema = Variable.get("mssql_staging_schema", default_var = "crt"),
    delete_missing=bool(int(Variable.get("delete_missing_price_disc_table", default_var=0))),
    delete_scope_column='RECID',
    batch_size = int(Variable.get("batch_size_price_disc_table", default_var = 30000))
)

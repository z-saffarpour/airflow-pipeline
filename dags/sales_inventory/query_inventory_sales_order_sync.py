...
Kafka Topics:
    - KAFKA_TOPIC  : Sales order quantities (dynamic date mapping)
...
BATCH_SIZE = int(Variable.get("batch_size_inventory_sales_order", default_var = 100000))   
...
def process_sales_order_per_date_task(exec_date: str, version: str, **context) -> Dict[str, Any]:
    ...
    Args:
        ...
        version: Version timestamp for this DAG run (YYYYMMDDHHMMSS).
...
with DAG(
    dag_id="query_inventory_sales_order_sync",
    description="Daily sync of sales order data to Kafka.",
    ...
    #     completion = dag_completion_task(process_sales_order)
        
    #     # Internal dependencies
    #     completion
        
    # Define task dependencies
    validation >> Setup >> discovery >> processing
...
Only ``sync_postgresql_to_mssql_chunk`` uses ``dag_config.pool``.
...
from pipeline.core.PostgreSQLToMSSQLQueryOrchestrator import PostgreSQLToMSSQLQueryOrchestrator
...
def make_sync_postgresql_to_mssql_task(
    dag_config: DAGConfig,
    sync_config: MasterDataSyncConfig,
    conn_config: ConnectionConfig,
):
    @task(
        task_id="sync_postgresql_to_mssql",
    ...
    def sync_postgresql_to_mssql(**context) -> Dict[str, Any]:
        audit = AuditLogger(
            dag_id=context["dag"].dag_id,
            run_id=context["run_id"],
        )
        ...
        try:
            orchestrator = PostgreSQLToMSSQLQueryOrchestrator(
                source_conn_id=conn_config.postgres_conn_id,
            ...
            if not result.success:
                ...
                message = f"PostgreSQL→MSSQL sync failed: {result.error_message}"
            ...
            logger.info(f"[postgresql_to_mssql_sync_dag_factory.sync_postgresql_to_mssql] PostgreSQL→MSSQL sync completed: {result.records_transferred} rows in {result.duration_seconds:.2f}s")
            return {
                "success": True,
                "source": sync_config.source_name,
                "records": result.records_transferred,
            ...
            raise_sync_task_error(f"PostgreSQL→MSSQL sync failed: {exc}", exc)

    return sync_postgresql_to_mssql
...
def make_create_sync_chunks_task(
    ...
    def create_sync_chunks(**context) -> List[Dict[str, Any]]:
        ...
        logger.info(f"[postgresql_to_mssql_sync_dag_factory.create_sync_chunks] Created {len(chunks)} sync chunks for table={sync_config.target_table}")
        return chunks

    return create_sync_chunks


def make_sync_postgresql_to_mssql_chunk_task(
    dag_config: DAGConfig,
    sync_config: MasterDataSyncConfig,
    conn_config: ConnectionConfig,
    chunk_pool: str,
    ...
    @task(
        task_id="sync_postgresql_to_mssql_chunk",
        retries=dag_config.sync_retries,
        retry_delay=dag_config.sync_retry_delay,
        execution_timeout=dag_config.execution_timeout,
    ...
    def sync_postgresql_to_mssql_chunk(
        chunk: Dict[str, Any],
        **context,
    ) -> Dict[str, Any]:
        audit = AuditLogger(
    ...
    return sync_postgresql_to_mssql_chunk
...
def make_report_sync_metrics_task():
    ...
    def report_sync_metrics(results: Any) -> Dict[str, Any]:
        ...
        logger.info("[postgresql_to_mssql_sync_dag_factory.report_sync_metrics] ----- PostgreSQL → MSSQL Sync Metrics -----")
        for row in result_rows:
            ...
            logger.info(f'[postgresql_to_mssql_sync_dag_factory.report_sync_metrics] chunk/source={row.get("chunk_no", row.get("source"))} -> inserted={inserted}, updated={updated}, deleted={deleted}, records={records}, duration={duration:.2f}s')

        logger.info(f"[postgresql_to_mssql_sync_dag_factory.report_sync_metrics] TOTAL -> inserted={total_inserted}, updated={total_updated}, deleted={total_deleted}, records={total_records}, duration={total_duration:.2f}s")
        return {
...
def create_dag(
    ...
    """
    Create a PostgreSQL → MSSQL sync DAG.

    Args:
        dag_config: Airflow DAG settings
        sync_config: Source query + target table upsert settings (MasterDataSyncConfig)
        conn_config: Must set postgres_conn_id (source) and mssql_conn_id (target)
    """
    if not conn_config.postgres_conn_id:
        raise ValueError("conn_config.postgres_conn_id is required for PostgreSQL→MSSQL sync")
    if not conn_config.mssql_conn_id:
        raise ValueError("conn_config.mssql_conn_id is required for PostgreSQL→MSSQL sync")
    ...
    with DAG(
        ...
        or "Sync PostgreSQL source query data to MSSQL target (upsert)",
        ...
        with TaskGroup(group_id="validation") as validation:
            ...
            [validate_postgres, validate_mssql]
        ...
        with TaskGroup(group_id="processing") as processing:
            ...
            if sync_config.use_dynamic_tasks:
                ...
                sync_chunk_task = make_sync_postgresql_to_mssql_chunk_task(
                ...
                sync_task = make_sync_postgresql_to_mssql_task(
                    dag_config=dag_config,
                    sync_config=sync_config,
                    conn_config=conn_config,
                )

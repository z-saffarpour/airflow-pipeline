...
def make_sync_mssql_to_mysql_task(
    ...
    def sync_mssql_to_mysql(**context) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "MSSQL→MySQL sync completed: %s rows in %.2fs",
...
def make_report_sync_metrics_task():
    ...
    def report_sync_metrics(results: Any) -> Dict[str, Any]:
        ...
        logger.info("----- MSSQL → MySQL Sync Metrics -----")
...
def create_dag(
    ...
    """
    Create an MSSQL → MySQL replication-style sync DAG.
    ...
    with DAG(
        ...
        or "Sync MSSQL source query data to MySQL target (upsert/replication-style)",

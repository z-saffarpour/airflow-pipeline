...
Only ``sync_mssql_to_mysql_chunk`` uses ``dag_config.pool``.
...
def make_sync_mssql_to_mysql_task(
    dag_config: DAGConfig,
    sync_config: MasterDataSyncConfig,
    conn_config: ConnectionConfig,
):
    @task(
        task_id="sync_mssql_to_mysql",
    ...
    def sync_mssql_to_mysql(**context) -> Dict[str, Any]:
        audit = AuditLogger(
            dag_id=context["dag"].dag_id,
            run_id=context["run_id"],
        )
        ...
        try:
            orchestrator = MSSQLToMySQLQueryOrchestrator(
                source_conn_id=conn_config.mssql_conn_id,
                target_conn_id=conn_config.mysql_conn_id,
                fail_on_error=True,
            ...
            if not result.success:
                ...
                message = f"MSSQL→MySQL sync failed: {result.error_message}"
            ...
            logger.info(f"[mssql_to_mysql_sync_dag_factory.sync_mssql_to_mysql] MSSQL→MySQL sync completed: {result.records_transferred} rows in {result.duration_seconds:.2f}s")
            return {
                "success": True,
                "source": sync_config.source_name,
                "records": result.records_transferred,
            ...
            raise_sync_task_error(f"MSSQL→MySQL sync failed: {exc}", exc)

    return sync_mssql_to_mysql
...
def make_create_sync_chunks_task(
    ...
    def create_sync_chunks(**context) -> List[Dict[str, Any]]:
        ...
        logger.info(f"[mssql_to_mysql_sync_dag_factory.create_sync_chunks] Created {len(chunks)} sync chunks for table={sync_config.target_table}")
        return chunks

    return create_sync_chunks


def make_sync_mssql_to_mysql_chunk_task(
    dag_config: DAGConfig,
    sync_config: MasterDataSyncConfig,
    conn_config: ConnectionConfig,
    chunk_pool: str,
    ...
    @task(
        task_id="sync_mssql_to_mysql_chunk",
        retries=dag_config.sync_retries,
        retry_delay=dag_config.sync_retry_delay,
        execution_timeout=dag_config.execution_timeout,
    ...
    return sync_mssql_to_mysql_chunk
...
def make_report_sync_metrics_task():
    ...
    def report_sync_metrics(results: Any) -> Dict[str, Any]:
        ...
        logger.info("[mssql_to_mysql_sync_dag_factory.report_sync_metrics] ----- MSSQL → MySQL Sync Metrics -----")
        for row in result_rows:
            ...
            logger.info(f'[mssql_to_mysql_sync_dag_factory.report_sync_metrics] chunk/source={row.get("chunk_no", row.get("source"))} -> inserted={inserted}, updated={updated}, deleted={deleted}, records={records}, duration={duration:.2f}s')

        logger.info(f"[mssql_to_mysql_sync_dag_factory.report_sync_metrics] TOTAL -> inserted={total_inserted}, updated={total_updated}, deleted={total_deleted}, records={total_records}, duration={total_duration:.2f}s")
        return {
...
def create_dag(
    ...
    """
    Create an MSSQL → MySQL sync DAG.

    Args:
        dag_config: Airflow DAG settings
        sync_config: Source query + target table upsert settings (MasterDataSyncConfig)
        conn_config: Must set mssql_conn_id (source) and mysql_conn_id (target)
    """
    if not conn_config.mssql_conn_id:
        raise ValueError("conn_config.mssql_conn_id is required for MSSQL→MySQL sync")
    if not conn_config.mysql_conn_id:
        raise ValueError("conn_config.mysql_conn_id is required for MSSQL→MySQL sync")
    ...
    with DAG(
        ...
        or "Sync MSSQL source query data to MySQL target (upsert)",
        ...
        with TaskGroup(group_id="processing") as processing:
            ...
            if sync_config.use_dynamic_tasks:
                ...
                sync_chunk_task = make_sync_mssql_to_mysql_chunk_task(
                ...
                sync_task = make_sync_mssql_to_mysql_task(
                    dag_config=dag_config,
                    sync_config=sync_config,
                    conn_config=conn_config,
                )

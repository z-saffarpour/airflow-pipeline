"""
MSSQL → ClickHouse sync DAG factory.

Reads data from an MSSQL source (query) and bulk-inserts into a ClickHouse target
table (ReplacingMergeTree-friendly INSERT with optional version_id).
Chunk planning uses SQL Server NTILE on the source when use_dynamic_tasks=True.
...
Only ``sync_mssql_to_clickhouse_chunk`` uses ``dag_config.pool``.
Light tasks (validation, create_chunks, report) use ``default_pool``.

Required Airflow connections:
    - mssql_*       : source (e.g. mssql_default)
    - clickhouse_*  : target (e.g. clickhouse_default)

Notes:
    - target_schema in MasterDataSyncConfig = ClickHouse database name
    - delete_missing / staging / hash-change detection are not applied on this path
"""
import logging
...
from pipeline.utils.validation import validate_mssql_conn, validate_clickhouse_conn
...
def make_validate_clickhouse_connection_task(conn_config: ConnectionConfig):
    @task(
    ...
    def validate_clickhouse_connection(**context):
        audit = AuditLogger(
            dag_id=context["dag"].dag_id,
            run_id=context["run_id"],
        )
        task_id = context["task_instance"].task_id
        conn_id = conn_config.clickhouse_conn_id

        audit.log(
            EventType.CONN_VALIDATED,
            task_id,
            EventStatus.STARTED,
            {"conn_id": conn_id, "type": "clickhouse"},
        )

        result = validate_clickhouse_conn(conn_id)
...
def make_sync_mssql_to_clickhouse_task(
    dag_config: DAGConfig,
    sync_config: MasterDataSyncConfig,
    conn_config: ConnectionConfig,
):
    @task(
        task_id="sync_mssql_to_clickhouse",
    ...
    def sync_mssql_to_clickhouse(**context) -> Dict[str, Any]:
        audit = AuditLogger(
            dag_id=context["dag"].dag_id,
            run_id=context["run_id"],
        )
        ...
        try:
            orchestrator = MSSQLToClickHouseQueryOrchestrator(
                source_conn_id=conn_config.mssql_conn_id,
                target_conn_id=conn_config.clickhouse_conn_id,
                fail_on_error=True,
            ...
            if not result.success:
                ...
                message = f"MSSQL→ClickHouse sync failed: {result.error_message}"
            ...
            logger.info(f"[mssql_to_clickhouse_sync_dag_factory.sync_mssql_to_clickhouse] MSSQL→ClickHouse sync completed: {result.records_transferred} rows in {result.duration_seconds:.2f}s")
            return {
                "success": True,
                "source": sync_config.source_name,
                "records": result.records_transferred,
            ...
            raise_sync_task_error(f"MSSQL→ClickHouse sync failed: {exc}", exc)

    return sync_mssql_to_clickhouse
    ...
    def create_sync_chunks(**context) -> List[Dict[str, Any]]:
        ...
        audit.log(
            ...
            EventStatus.STARTED,
            {
                ...
                "chunk_column": sync_config.chunk_column or (
                    sync_config.primary_keys[0] if sync_config.primary_keys else None
                ),
            },
        )
        ...
        logger.info(f"[mssql_to_clickhouse_sync_dag_factory.create_sync_chunks] Created {len(chunks)} sync chunks for table={sync_config.target_table}")
        return chunks

    return create_sync_chunks


def make_sync_mssql_to_clickhouse_chunk_task(
    dag_config: DAGConfig,
    sync_config: MasterDataSyncConfig,
    conn_config: ConnectionConfig,
    chunk_pool: str,
    ...
    @task(
        task_id="sync_mssql_to_clickhouse_chunk",
        retries=dag_config.sync_retries,
        retry_delay=dag_config.sync_retry_delay,
        execution_timeout=dag_config.execution_timeout,
    ...
    def sync_mssql_to_clickhouse_chunk(
        chunk: Dict[str, Any],
        **context,
    ) -> Dict[str, Any]:
        audit = AuditLogger(
    ...
    return sync_mssql_to_clickhouse_chunk
...
def make_report_sync_metrics_task():
    ...
    def report_sync_metrics(results: Any) -> Dict[str, Any]:
        ...
        logger.info("[mssql_to_clickhouse_sync_dag_factory.report_sync_metrics] ----- MSSQL → ClickHouse Sync Metrics -----")
        for row in result_rows:
            ...
            logger.info(f'[mssql_to_clickhouse_sync_dag_factory.report_sync_metrics] chunk/source={row.get("chunk_no", row.get("source"))} -> inserted={inserted}, updated={updated}, deleted={deleted}, records={records}, duration={duration:.2f}s')

        logger.info(f"[mssql_to_clickhouse_sync_dag_factory.report_sync_metrics] TOTAL -> inserted={total_inserted}, updated={total_updated}, deleted={total_deleted}, records={total_records}, duration={total_duration:.2f}s")
        return {
...
def create_dag(
    ...
    """
    Create an MSSQL → ClickHouse sync DAG.

    Args:
        dag_config: Airflow DAG settings
        sync_config: Source query + ClickHouse target settings (MasterDataSyncConfig)
        conn_config: Must set mssql_conn_id (source) and clickhouse_conn_id (target)
    """
    if not conn_config.mssql_conn_id:
        raise ValueError("conn_config.mssql_conn_id is required for MSSQL→ClickHouse sync")
    if not conn_config.clickhouse_conn_id:
        raise ValueError(
            "conn_config.clickhouse_conn_id is required for MSSQL→ClickHouse sync"
        )
    ...
    with DAG(
        ...
        or "Sync MSSQL source query data to ClickHouse (bulk INSERT / ReplacingMergeTree)",
        ...
        with TaskGroup(group_id="validation") as validation:
            ...
            validate_clickhouse = make_validate_clickhouse_connection_task(conn_config)()
            [validate_mssql, validate_clickhouse]
        ...
        with TaskGroup(group_id="processing") as processing:
            ...
            if sync_config.use_dynamic_tasks:
                ...
                sync_chunk_task = make_sync_mssql_to_clickhouse_chunk_task(
                ...
                sync_task = make_sync_mssql_to_clickhouse_task(
                    dag_config=dag_config,
                    sync_config=sync_config,
                    conn_config=conn_config,
                )

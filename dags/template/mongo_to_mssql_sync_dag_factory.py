...
def make_sync_mongo_to_mssql_task(
    ...
    def sync_mongo_to_mssql(**context) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"[mongo_to_mssql_sync_dag_factory.sync_mongo_to_mssql] MongoDB→MSSQL sync completed: {result.records_transferred} rows in {result.duration_seconds:.2f}s")
            return {
                "success": True,
                "source": sync_config.source_name,
                "records": result.records_transferred,
...
def make_create_sync_chunks_task(
    ...
    def create_sync_chunks(**context) -> List[Dict[str, Any]]:
        ...
        logger.info(f"[mongo_to_mssql_sync_dag_factory.create_sync_chunks] Created {len(chunks)} sync chunks for table={sync_config.target_table}")
        return chunks
...
def make_report_sync_metrics_task():
    ...
    def report_sync_metrics(results: Any) -> Dict[str, Any]:
        ...
        logger.info("[mongo_to_mssql_sync_dag_factory.report_sync_metrics] ----- MongoDB → MSSQL Sync Metrics -----")
        for row in result_rows:
            ...
            logger.info(f'[mongo_to_mssql_sync_dag_factory.report_sync_metrics] chunk/source={row.get("chunk_no", row.get("source"))} -> inserted={inserted}, updated={updated}, deleted={deleted}, records={records}, duration={duration:.2f}s')

        logger.info(f"[mongo_to_mssql_sync_dag_factory.report_sync_metrics] TOTAL -> inserted={total_inserted}, updated={total_updated}, deleted={total_deleted}, records={total_records}, duration={total_duration:.2f}s")
        return {

...
def make_sync_mysql_to_mssql_task(
    ...
    def sync_mysql_to_mssql(**context) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "MySQL→MSSQL sync completed: %s rows in %.2fs",
...
def make_report_sync_metrics_task():
    ...
    def report_sync_metrics(results: Any) -> Dict[str, Any]:
        ...
        logger.info("----- MySQL → MSSQL Sync Metrics -----")
...
def create_dag(
    ...
    """
    Create a MySQL → MSSQL replication-style sync DAG.
    ...
    with DAG(
        ...
        or "Sync MySQL source query data to MSSQL target (upsert/replication-style)",

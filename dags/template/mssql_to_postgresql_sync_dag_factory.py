"""
MSSQL → PostgreSQL sync DAG factory.

Reads data from an MSSQL source (query) and upserts into a PostgreSQL target table,
mirroring the MSSQL→MySQL pattern (staging upsert, optional chunks,
...
Only ``sync_mssql_to_postgresql_chunk`` uses ``dag_config.pool``.
...
from pipeline.utils.validation import validate_mssql_conn, validate_postgres_conn
...
def make_validate_postgres_connection_task(conn_config: ConnectionConfig):
    @task(
    ...
    def validate_postgres_connection(**context):
        audit = AuditLogger(
            dag_id=context["dag"].dag_id,
            run_id=context["run_id"],
        )
        task_id = context["task_instance"].task_id
        conn_id = conn_config.postgres_conn_id

        audit.log(
            EventType.CONN_VALIDATED,
            task_id,
            EventStatus.STARTED,
            {"conn_id": conn_id, "type": "postgres"},
        )

        result = validate_postgres_conn(conn_id)
...
def make_sync_mssql_to_postgresql_task(
    dag_config: DAGConfig,
    sync_config: MasterDataSyncConfig,
    conn_config: ConnectionConfig,
):
    @task(
        task_id="sync_mssql_to_postgresql",
    ...
    def sync_mssql_to_postgresql(**context) -> Dict[str, Any]:
        audit = AuditLogger(
            dag_id=context["dag"].dag_id,
            run_id=context["run_id"],
        )
        ...
        try:
            orchestrator = MSSQLToPostgreSQLQueryOrchestrator(
                source_conn_id=conn_config.mssql_conn_id,
                target_conn_id=conn_config.postgres_conn_id,
                fail_on_error=True,
            ...
            if not result.success:
                ...
                message = f"MSSQL→PostgreSQL sync failed: {result.error_message}"
            ...
            logger.info(f"[mssql_to_postgresql_sync_dag_factory.sync_mssql_to_postgresql] MSSQL→PostgreSQL sync completed: {result.records_transferred} rows in {result.duration_seconds:.2f}s")
            return {
                "success": True,
                "source": sync_config.source_name,
                "records": result.records_transferred,
            ...
            raise_sync_task_error(f"MSSQL→PostgreSQL sync failed: {exc}", exc)

    return sync_mssql_to_postgresql
...
def make_create_sync_chunks_task(
    ...
    def create_sync_chunks(**context) -> List[Dict[str, Any]]:
        ...
        logger.info(f"[mssql_to_postgresql_sync_dag_factory.create_sync_chunks] Created {len(chunks)} sync chunks for table={sync_config.target_table}")
        return chunks

    return create_sync_chunks


def make_sync_mssql_to_postgresql_chunk_task(
    dag_config: DAGConfig,
    sync_config: MasterDataSyncConfig,
    conn_config: ConnectionConfig,
    chunk_pool: str,
    ...
    @task(
        task_id="sync_mssql_to_postgresql_chunk",
        retries=dag_config.sync_retries,
        retry_delay=dag_config.sync_retry_delay,
        execution_timeout=dag_config.execution_timeout,
    ...
    def sync_mssql_to_postgresql_chunk(
        chunk: Dict[str, Any],
        **context,
    ) -> Dict[str, Any]:
        audit = AuditLogger(
    ...
    return sync_mssql_to_postgresql_chunk
...
def make_report_sync_metrics_task():
    ...
    def report_sync_metrics(results: Any) -> Dict[str, Any]:
        ...
        logger.info("[mssql_to_postgresql_sync_dag_factory.report_sync_metrics] ----- MSSQL → PostgreSQL Sync Metrics -----")
        for row in result_rows:
            ...
            logger.info(f'[mssql_to_postgresql_sync_dag_factory.report_sync_metrics] chunk/source={row.get("chunk_no", row.get("source"))} -> inserted={inserted}, updated={updated}, deleted={deleted}, records={records}, duration={duration:.2f}s')

        logger.info(f"[mssql_to_postgresql_sync_dag_factory.report_sync_metrics] TOTAL -> inserted={total_inserted}, updated={total_updated}, deleted={total_deleted}, records={total_records}, duration={total_duration:.2f}s")
        return {
...
def create_dag(
    ...
    """
    Create an MSSQL → PostgreSQL sync DAG.

    Args:
        dag_config: Airflow DAG settings
        sync_config: Source query + target table upsert settings (MasterDataSyncConfig)
        conn_config: Must set mssql_conn_id (source) and postgres_conn_id (target)
    """
    if not conn_config.mssql_conn_id:
        raise ValueError("conn_config.mssql_conn_id is required for MSSQL→PostgreSQL sync")
    if not conn_config.postgres_conn_id:
        raise ValueError("conn_config.postgres_conn_id is required for MSSQL→PostgreSQL sync")
    ...
    with DAG(
        ...
        or "Sync MSSQL source query data to PostgreSQL target (upsert)",
        ...
        with TaskGroup(group_id="validation") as validation:
            ...
            validate_postgres = make_validate_postgres_connection_task(conn_config)()
            [validate_mssql, validate_postgres]
        ...
        with TaskGroup(group_id="processing") as processing:
            ...
            if sync_config.use_dynamic_tasks:
                ...
                sync_chunk_task = make_sync_mssql_to_postgresql_chunk_task(
                ...
                sync_task = make_sync_mssql_to_postgresql_task(
                    dag_config=dag_config,
                    sync_config=sync_config,
                    conn_config=conn_config,
                )

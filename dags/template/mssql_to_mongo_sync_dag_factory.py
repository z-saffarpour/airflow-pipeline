...
Convention:
    - MasterDataSyncConfig.target_schema → MongoDB database name
    - MasterDataSyncConfig.target_table  → MongoDB collection name
...
Only ``sync_mssql_to_mongo_chunk`` uses ``dag_config.pool``.
...
def make_sync_mssql_to_mongo_task(
    dag_config: DAGConfig,
    sync_config: MasterDataSyncConfig,
    conn_config: ConnectionConfig,
):
    @task(
        task_id="sync_mssql_to_mongo",
    ...
    def sync_mssql_to_mongo(**context) -> Dict[str, Any]:
        audit = AuditLogger(
            dag_id=context["dag"].dag_id,
            run_id=context["run_id"],
        )
        ...
        try:
            orchestrator = MSSQLToMongoDBQueryOrchestrator(
                source_conn_id=conn_config.mssql_conn_id,
                target_conn_id=conn_config.mongo_conn_id,
                fail_on_error=True,
            ...
            if not result.success:
                ...
                message = f"MSSQL→Mongo sync failed: {result.error_message}"
            ...
            raise_sync_task_error(f"MSSQL→Mongo sync failed: {exc}", exc)

    return sync_mssql_to_mongo
...
def make_sync_mssql_to_mongo_chunk_task(
    dag_config: DAGConfig,
    sync_config: MasterDataSyncConfig,
    conn_config: ConnectionConfig,
    chunk_pool: str,
    ...
    @task(
        task_id="sync_mssql_to_mongo_chunk",
        retries=dag_config.sync_retries,
        retry_delay=dag_config.sync_retry_delay,
        execution_timeout=dag_config.execution_timeout,
    ...
    return sync_mssql_to_mongo_chunk
...
def create_dag(
    ...
    """
    Create an MSSQL → MongoDB replication-style sync DAG.

    Args:
        dag_config: Airflow DAG settings
        sync_config: Source query + target collection upsert settings
            (MasterDataSyncConfig; target_schema=database, target_table=collection)
        conn_config: Must set mssql_conn_id (source) and mongo_conn_id (target)
    """
    if not conn_config.mssql_conn_id:
        raise ValueError("conn_config.mssql_conn_id is required for MSSQL→Mongo sync")
    if not conn_config.mongo_conn_id:
        raise ValueError("conn_config.mongo_conn_id is required for MSSQL→Mongo sync")
    ...
    with DAG(
        ...
        or "Sync MSSQL source query data to MongoDB target (upsert/replication-style)",
        ...
        with TaskGroup(group_id="processing") as processing:
            ...
            if sync_config.use_dynamic_tasks:
                ...
                sync_chunk_task = make_sync_mssql_to_mongo_chunk_task(
                ...
                sync_task = make_sync_mssql_to_mongo_task(
                    dag_config=dag_config,
                    sync_config=sync_config,
                    conn_config=conn_config,
                )

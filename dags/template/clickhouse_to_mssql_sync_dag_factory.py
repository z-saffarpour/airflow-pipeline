...
def make_sync_clickhouse_to_mssql_task(
    ...
    def sync_clickhouse_to_mssql(**context) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "ClickHouse→MSSQL sync completed: %s rows in %.2fs",
...
def make_report_sync_metrics_task():
    ...
    def report_sync_metrics(results: Any) -> Dict[str, Any]:
        ...
        logger.info("----- ClickHouse → MSSQL Sync Metrics -----")
...
def create_dag(
    ...
    """
    Create a ClickHouse → MSSQL replication-style sync DAG.
    ...
    with DAG(
        ...
        or "Sync ClickHouse source query data to MSSQL target (upsert/replication-style)",

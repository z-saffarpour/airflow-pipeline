...
class IdempotentKafkaProducer(MessageProducer):
    ...
    def produce(
        ...
        try:
            ...
            self.logger.warning(
                "Producer queue is full, polling to clear delivery queue before retry",
                extra={
                    "topic": topic,
            ...
            try:
                ...
                self.logger.error(
                    "Failed to produce message after retry",
                    extra={
                        "topic": topic,
    ...
    def flush(self, timeout: float = KAFKA_CONFIG.DEFAULT_FLUSH_TIMEOUT) -> None:
        ...
        self.logger.debug(f"Flushing producer with timeout={timeout}s")
    ...
    def close(self, timeout: float = KAFKA_CONFIG.DEFAULT_FLUSH_TIMEOUT) -> None:
        ...
        try:
            ...
            self.logger.error("Error during producer close", exc_info=True)
            raise
        finally:

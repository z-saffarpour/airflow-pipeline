"""
Orchestrator for syncing query results from MSSQL to MongoDB (upsert).
"""
import logging
import math
import re
...
class MSSQLToMongoDBQueryOrchestrator(SyncOrchestrator):
    """
    Read batches from MSSQL and upsert into MongoDB collections.
    Mirrors MSSQLToMySQLQueryOrchestrator with MongoDB as the target.
    """
    ...
    def _create_writer(self) -> MongoDBServerWriter:
        return MongoDBServerWriter(
            conn_id=self.target_conn_id,
            id_as_mongo_id=self.id_as_mongo_id,
        )
    ...
    def sync_data_chunk(
        ...
        """Sync one key-range chunk from MSSQL to MongoDB."""

"""
Orchestrator for syncing query results from PostgreSQL to MSSQL (upsert).
"""
import logging
import math
import re
...
class PostgreSQLToMSSQLQueryOrchestrator(SyncOrchestrator):
    """
    Read batches from PostgreSQL and upsert into MSSQL target tables.
    Mirrors MySQLToMSSQLQueryOrchestrator behaviour with PostgreSQL source dialect.
    """
    ...
    def build_chunk_plan_query(
        ...
        """Build NTILE-based chunk plan (PostgreSQL)."""
    ...
    def plan_sync_chunks(
        ...
        """Build key ranges for parallel dynamic sync tasks (PostgreSQL NTILE)."""
    ...
    def sync_data_chunk(
        ...
        """Sync one key-range chunk from PostgreSQL to MSSQL."""

...
class MSSQLToMSSQLQueryOrchestrator(SyncOrchestrator):
    ...
    def _resolve_delete_scope_column(sync_config: MasterDataSyncConfig) -> str:
        """Column used to bound scoped delete_missing (independent of primary_keys)."""
    ...
    def _finalize_delete_missing(
        ...
        """Run scoped orphan delete and always drop the keys staging table."""
        try:
    ...
    def plan_sync_chunks(
        ...
        """Build RECID/key ranges for parallel dynamic sync tasks."""
    ...
    def sync_data_chunk(
        ...
        """Sync one key range to the store using per-batch upsert (no bulk staging)."""

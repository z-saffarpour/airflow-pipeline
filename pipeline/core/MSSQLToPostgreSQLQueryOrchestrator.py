...
class MSSQLToPostgreSQLQueryOrchestrator:
    ...
    def _finalize_delete_missing(
        ...
        try:
            if min_key is None or max_key is None:
                self.logger.info(
                    "[MSSQLToPostgreSQLQueryOrchestrator._finalize_delete_missing] Skipping "
            ...
            self.logger.info(
                "[MSSQLToPostgreSQLQueryOrchestrator._finalize_delete_missing] Scoped delete "
    ...
    def plan_sync_chunks(
        ...
        if total_count == 0:
            self.logger.info(
                "[MSSQLToPostgreSQLQueryOrchestrator.plan_sync_chunks] No rows to plan | table=%s",
                sync_config.target_table,
            )
            return []
        ...
        self.logger.info(
            "[MSSQLToPostgreSQLQueryOrchestrator.plan_sync_chunks] Planned %d chunks | "
    ...
    def sync_data_chunk(
        ...
        self.logger.info(
            "[MSSQLToPostgreSQLQueryOrchestrator.sync_data_chunk] Starting | table=%s | "
    ...
    def sync_data(
        ...
        self.logger.info(
            "[MSSQLToPostgreSQLQueryOrchestrator.sync_data] Starting MSSQL → PostgreSQL sync | "
        ...
        try:
            ...
            for batch in reader.stream_query(
                ...
                self.logger.info(
                    "[MSSQLToPostgreSQLQueryOrchestrator.sync_data] Batch %s | rows=%s | "
                    "transferred=%s",
                    batch_number,
                    batch_size,
            ...
            metrics.mark_completed()
            self.logger.info(
                "[MSSQLToPostgreSQLQueryOrchestrator.sync_data] Completed | table=%s | "
            ...
            self.logger.error(
                "[MSSQLToPostgreSQLQueryOrchestrator.sync_data] Failed | table=%s | error=%s",
                sync_config.source_name,
                exc,
                exc_info=True,
            )

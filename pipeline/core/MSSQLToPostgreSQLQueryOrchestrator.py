"""
Orchestrator for syncing query results from MSSQL to PostgreSQL (upsert).
"""
import logging
import math
import re
...
class MSSQLToPostgreSQLQueryOrchestrator(SyncOrchestrator):
    ...
    def build_scope_bounds_query(
        ...
        return f"""
        SELECT
            MIN([{chunk_column}]) AS min_key,
            MAX([{chunk_column}]) AS max_key
        FROM (
            {inner_query}
        ) AS source_data
        """
    ...
    def build_chunk_delete_scope_bounds_query(
        ...
        return f"""
        SELECT
            MIN([{delete_scope_column}]) AS min_key,
            MAX([{delete_scope_column}]) AS max_key
        FROM (
            {inner_query}
        ) AS source_data
    ...
    def _create_writer(self) -> PostgreSQLServerWriter:
        return PostgreSQLServerWriter(conn_id=self.target_conn_id)
    ...
    def _finalize_delete_missing(
        ...
        try:
            if min_key is None or max_key is None:
                self.logger.info(
                    f"[MSSQLToPostgreSQLQueryOrchestrator._finalize_delete_missing] Skipping scoped delete | "
            ...
            self.logger.info(
                f"[MSSQLToPostgreSQLQueryOrchestrator._finalize_delete_missing] Scoped delete completed | "
    ...
    def plan_sync_chunks(
        ...
        if total_count == 0:
            self.logger.info(
                f"[MSSQLToPostgreSQLQueryOrchestrator.plan_sync_chunks] No rows to plan | table={sync_config.target_table}"
            )
            return []
        ...
        self.logger.info(
            f"[MSSQLToPostgreSQLQueryOrchestrator.plan_sync_chunks] Planned {len(chunks)} chunks | "
    ...
    def sync_data_chunk(
        ...
        """Sync one key-range chunk from MSSQL to PostgreSQL."""
        ...
        self.logger.info(
            f"[MSSQLToPostgreSQLQueryOrchestrator.sync_data_chunk] Starting | table={sync_config.target_table} | "
    ...
    def sync_data(
        ...
        self.logger.info(
            f"[MSSQLToPostgreSQLQueryOrchestrator.sync_data] Starting MSSQL → PostgreSQL sync | "
        ...
        try:
            ...
            for batch in reader.stream_query(
                ...
                self.logger.info(
                    f"[MSSQLToPostgreSQLQueryOrchestrator.sync_data] Batch {batch_number} | rows={batch_size} | transferred={metrics.transferred_records}"
                )
            ...
            metrics.mark_completed()
            self.logger.info(
                f"[MSSQLToPostgreSQLQueryOrchestrator.sync_data] Completed | table={sync_config.source_name} | "
            ...
            self.logger.error(
                f"[MSSQLToPostgreSQLQueryOrchestrator.sync_data] Failed | table={sync_config.source_name} | error={exc}",
                exc_info=True
            )
            metrics.mark_failed(error_message)
            return TransferResult.create_failure(

...
class MySQLToMSSQLQueryOrchestrator(SyncOrchestrator):
    ...
    def _finalize_delete_missing(
        ...
        try:
            if min_key is None or max_key is None:
                self.logger.info(
                    "[MySQLToMSSQLQueryOrchestrator._finalize_delete_missing] Skipping scoped delete | "
            ...
            self.logger.info(
                "[MySQLToMSSQLQueryOrchestrator._finalize_delete_missing] Scoped delete completed | "
                "table=%s | deleted=%s",
                sync_config.target_table,
                deleted,
            )
    ...
    def plan_sync_chunks(
        ...
        if total_count == 0:
            self.logger.info(
                "[MySQLToMSSQLQueryOrchestrator.plan_sync_chunks] No rows to plan | table=%s",
                sync_config.target_table,
            )
            return []
        ...
        self.logger.info(
            "[MySQLToMSSQLQueryOrchestrator.plan_sync_chunks] Planned %d chunks | table=%s | total_rows=%s",
            len(chunks),
            sync_config.target_table,
            total_count,
        )
    ...
    def sync_data_chunk(
        ...
        self.logger.info(
            "[MySQLToMSSQLQueryOrchestrator.sync_data_chunk] Starting | table=%s | chunk=%s | "
    ...
    def sync_data(
        ...
        self.logger.info(
            "[MySQLToMSSQLQueryOrchestrator.sync_data] Starting MySQL → MSSQL sync | "
        ...
        try:
            ...
            metrics.mark_completed()
            self.logger.info(
                "[MySQLToMSSQLQueryOrchestrator.sync_data] Completed | table=%s | "
            ...
            self.logger.error(
                "[MySQLToMSSQLQueryOrchestrator.sync_data] Failed | table=%s | error=%s",
                sync_config.source_name,
                exc,
                exc_info=True,
            )

"""
Orchestrator for syncing query results from MySQL to MSSQL (upsert).
"""
import logging
import math
import re
...
class MySQLToMSSQLQueryOrchestrator(SyncOrchestrator):
    """
    Read batches from MySQL and upsert into MSSQL target tables.
    Mirrors MSSQLToMSSQLQueryOrchestrator behaviour with MySQL source dialect.
    """
    ...
    def build_chunk_plan_query(
        ...
        """Build NTILE-based chunk plan (MySQL 8+)."""
    ...
    def build_chunk_delete_scope_bounds_query(
        ...
        WHERE source_data.{quoted_chunk} >= %s
          AND source_data.{quoted_chunk} <= %s
        """
    ...
    def build_chunk_source_query(
        ...
        WHERE source_data.{quoted} >= %s
          AND source_data.{quoted} <= %s
        """
    ...
    def _fetch_scope_bounds(
        ...
        rows = reader.connection_factory.execute_query(bounds_query)
        if not rows:
            return None, None
        row = rows[0]
    ...
    def _fetch_chunk_delete_scope_bounds(
        ...
        rows = reader.connection_factory.execute_query(
            bounds_query,
            parameters=(min_key, max_key),
        )
        if not rows:
            return None, None
        row = rows[0]
    ...
    def _finalize_delete_missing(
        ...
        try:
            if min_key is None or max_key is None:
                self.logger.info(
                    f"[MySQLToMSSQLQueryOrchestrator._finalize_delete_missing] Skipping scoped delete | "
            ...
            self.logger.info(
                f"[MySQLToMSSQLQueryOrchestrator._finalize_delete_missing] Scoped delete completed | "
    ...
    def plan_sync_chunks(
        ...
        """Build key ranges for parallel dynamic sync tasks (MySQL 8+ NTILE)."""
        ...
        if total_count == 0:
            self.logger.info(
                f"[MySQLToMSSQLQueryOrchestrator.plan_sync_chunks] No rows to plan | table={sync_config.target_table}"
            )
            return []
        ...
        self.logger.info(
            f"[MySQLToMSSQLQueryOrchestrator.plan_sync_chunks] Planned {len(chunks)} chunks | "
    ...
    def sync_data_chunk(
        ...
        """Sync one key-range chunk from MySQL to MSSQL."""
        ...
        self.logger.info(
            f"[MySQLToMSSQLQueryOrchestrator.sync_data_chunk] Starting | table={sync_config.target_table} | "
        ...
        try:
            ...
            if sync_config.delete_missing and keys_staging_table:
                ...
                if delete_scope_column == chunk_column:
                    ...
                    delete_min_key, delete_max_key = self._fetch_chunk_delete_scope_bounds(
                        reader,
                        sync_config.source_query,
                        chunk_column,
                        min_key,
    ...
    def sync_data(
        ...
        self.logger.info(
            f"[MySQLToMSSQLQueryOrchestrator.sync_data] Starting MySQL → MSSQL sync | "
        ...
        try:
            ...
            for batch in reader.stream_query(
                ...
                self.logger.info(
                    f"[MySQLToMSSQLQueryOrchestrator.sync_data] Batch {batch_number} | rows={batch_size} | transferred={metrics.transferred_records}"
                )
            ...
            metrics.mark_completed()
            self.logger.info(
                f"[MySQLToMSSQLQueryOrchestrator.sync_data] Completed | table={sync_config.source_name} | "
            ...
            self.logger.error(
                f"[MySQLToMSSQLQueryOrchestrator.sync_data] Failed | table={sync_config.source_name} | error={exc}",
                exc_info=True
            )
            metrics.mark_failed(error_message)
            return TransferResult.create_failure(

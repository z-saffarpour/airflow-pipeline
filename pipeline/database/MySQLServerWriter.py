"""
MySQL Writer with batch upsert via staging tables.
Designed for Airflow ETL pipelines (MSSQL → MySQL sync and similar).
"""
import logging
...
def _is_mysql_deadlock(exc: BaseException) -> bool:
    """Return True if exc looks like a MySQL deadlock (errno 1213)."""
    args = getattr(exc, "args", ())
    if args and str(args[0]) in {"1213", "40001"}:
        return True
    message = str(exc).lower()
    return "1213" in message and "deadlock" in message


class MySQLServerWriter(DataWriter):
    """
    MySQL writer with batch upsert via staging tables.
    ...
    - Hash-based change detection (SHA2)
    ...
    def _discover_unique_keys(
        ...
        if unique_keys:
            self.logger.info(f"[MySQLServerWriter._discover_unique_keys] Discovered {len(unique_keys)} unique key(s) for {schema}.{table}: {unique_keys}")
        return unique_keys
    ...
    def _prepare_staging_table(self, cursor, staging_table: str, table_full: str) -> None:
        ...
        cursor.execute(f"CREATE TABLE {staging_table} LIKE {table_full}")
    ...
    def _cleanup_staging_table(self, conn, cursor, staging_table: str) -> None:
        try:
            ...
            if not self.autocommit:
                conn.rollback()
            self.logger.warning(f"[MySQLServerWriter._cleanup_staging_table] Failed to drop {staging_table}: {exc}")
    ...
    def _finalize_connection(self, conn, cursor, *, commit: bool = True) -> None:
        try:
            ...
            self.logger.warning(f"[MySQLServerWriter._finalize_connection] Commit/rollback failed: {exc}")
        try:
            cursor.close()
        except Exception:
            pass
    ...
    def delete_batch(
        ...
        FROM {table_full} AS target
        INNER JOIN {staging_table} AS source
            ON target.{quoted_key} = source.{quoted_key}
        """
        ...
        with self.connection_factory.get_connection() as conn:
            ...
            try:
                ...
                cursor.execute(
                    ...
                    f"SELECT {quoted_key} FROM {table_full} WHERE 1=0"
                )
        ...
        with self.connection_factory.get_connection() as conn:
            ...
            try:
                ...
                cursor.execute(
                    ...
                    f"SELECT {key_column_list} FROM {table_full} WHERE 1=0"
                )
                if not self.autocommit:
                    conn.commit()
                self.logger.info(f"[MySQLServerWriter.prepare_keys_staging_table] Prepared {keys_staging_table} for {table_full}")
    ...
    def delete_missing_in_scope(
        ...
        if min_key is None or max_key is None:
            self.logger.warning(f"[MySQLServerWriter.delete_missing_in_scope] Skipping scoped delete for {schema}.{table}; min_key or max_key is NULL")
            return 0
        ...
        with self.connection_factory.get_connection() as conn:
            ...
            try:
                ...
                if not self.autocommit:
                    conn.commit()
                self.logger.info(
                    f"[MySQLServerWriter.delete_missing_in_scope] Scoped delete for {table_full} | "
    ...
    def upsert_batch(
        ...
        if not data:
            self.logger.warning("[MySQLServerWriter.upsert_batch] No data to upsert")

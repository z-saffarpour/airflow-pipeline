...
class MySQLServerWriter(DataWriter):
    ...
    def _discover_unique_keys(
        ...
        if unique_keys:
            self.logger.info(
                "[MySQLServerWriter._discover_unique_keys] Discovered %d unique key(s) "
                "for %s.%s: %s",
                len(unique_keys),
                schema,
                table,
    ...
    def _delete_unique_key_conflicts(
        ...
        for unique_key in unique_keys:
            ...
            if deleted:
                ...
                self.logger.info(
                    "[MySQLServerWriter._delete_unique_key_conflicts] Deleted %d "
    ...
    def _cleanup_staging_table(self, conn, cursor, staging_table: str) -> None:
        try:
            ...
            if not self.autocommit:
                conn.rollback()
            self.logger.warning(
                "[MySQLServerWriter._cleanup_staging_table] Failed to drop %s: %s",
                staging_table,
                exc,
            )
    ...
    def _finalize_connection(self, conn, cursor, *, commit: bool = True) -> None:
        try:
            ...
            self.logger.warning(
                "[MySQLServerWriter._finalize_connection] Commit/rollback failed: %s",
                exc,
            )
        try:
            cursor.close()
        ...
        with self.connection_factory.get_connection() as conn:
            ...
            try:
                ...
                if not self.autocommit:
                    conn.commit()
                self.logger.info(
                    "[MySQLServerWriter.prepare_keys_staging_table] Prepared %s for %s",
                    keys_staging_table,
                    table_full,
                )
    ...
    def delete_missing_in_scope(
        ...
        if min_key is None or max_key is None:
            self.logger.warning(
                "[MySQLServerWriter.delete_missing_in_scope] Skipping scoped delete "
        ...
        with self.connection_factory.get_connection() as conn:
            ...
            try:
                ...
                if not self.autocommit:
                    conn.commit()
                self.logger.info(
                    "[MySQLServerWriter.delete_missing_in_scope] Scoped delete for %s | "
        ...
        while True:
            try:
                ...
                if attempt >= retry_ctx.max_attempts:
                    self.logger.error(
                        "[MySQLServerWriter.upsert_batch] Deadlock persisted after "
                        "%s attempts for %s: %s",
                    ...
                    raise
                self.logger.warning(
                    "[MySQLServerWriter.upsert_batch] Deadlock victim on attempt "
                    "%s/%s for %s, retrying...",
                    attempt,
                    retry_ctx.max_attempts,
                    table_full,

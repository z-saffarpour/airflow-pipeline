...
class MySQLConnectionFactory:
    ...
    def get_connection(self):
        ...
        try:
            ...
            self.logger.error(
                "[MySQLConnectionFactory.get_connection] Database connection failed | "
                "conn_id='%s' | error=%s",
                self.conn_id,
                exc,
                exc_info=True,
    ...
    def get_cursor(self, as_dict: bool = True):
        ...
        with self.get_connection() as connection:
            ...
            try:
                ...
                self.logger.error(
                    "[MySQLConnectionFactory.get_cursor] ERROR creating cursor | "
                    "conn_id='%s' | error=%s",
                    self.conn_id,
                    exc,
                    exc_info=True,
    ...
    def execute_query(
        ...
        self.logger.debug(
            "[MySQLConnectionFactory.execute_query] START | params_provided=%s",
        ...
        try:
            ...
            self.logger.error(
                "[MySQLConnectionFactory.execute_query] Query failed: %s",
                exc,
                exc_info=True,
            )
    ...
    def execute_scalar(
        ...
        try:
            ...
            self.logger.error(
                "[MySQLConnectionFactory.execute_scalar] Query failed: %s",
                exc,
                exc_info=True,
            )

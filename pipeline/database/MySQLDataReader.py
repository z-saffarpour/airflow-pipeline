...
class MySQLDataReader(DataReader):
    ...
    def get_total_count(
        ...
        self.logger.info(
            "[MySQLDataReader.get_total_count] START | mode=%s | table=%s",
            mode,
            table_name,
        )
        try:
            ...
            self.logger.info(
                "[MySQLDataReader.get_total_count] SUCCESS | count=%s",
                count,
            )
            return count
            ...
            self.logger.error(
                "[MySQLDataReader.get_total_count] Unexpected ERROR | error=%s",
                exc,
                exc_info=True,
            )
    ...
    def stream_data(
        ...
        if total_count == 0:
            self.logger.warning(
                "[MySQLDataReader.stream_data] NO DATA | table=%s",
                table_name,
            )
            return
    ...
    def stream_query(
        ...
        self.logger.info(
            "[MySQLDataReader.stream_query] START | batch_size=%s | has_count_query=%s",
            self.batch_size,
        ...
        if count_query is not None:
            ...
            self.logger.info(
                "[MySQLDataReader.stream_query] Total rows to stream: %s",
                f"{total_count:,}",
            )
        else:
            total_count = None
        ...
        try:
            with self.connection_factory.get_cursor(as_dict=True) as cursor:
                ...
                while True:
                    ...
                    if total_count:
                        ...
                        self.logger.info(
                            "[MySQLDataReader.stream_query] Batch %s | rows=%s | "
                            "processed=%s/%s (%.1f%%)",
                            batch_number,
                            len(batch),
                            f"{processed:,}",
            ...
            self.logger.error(
                "[MySQLDataReader.stream_query] ERROR | processed=%s | error=%s",
                processed,
                exc,
                exc_info=True,
            )
        ...
        self.logger.info(
            "[MySQLDataReader.stream_query] FINISH | processed=%s | batches=%s",
            processed,
            batch_number,
        )
